...
Agent 在归类阶段必须识别每一天的行程类型，因为它直接决定地图怎么画。

| `day_type` | 判断依据 | 地图展示 |
|------------|---------|---------|
| `drive` | 有租车/自驾；途经多个景点/山口/小镇；`travelmode: driving` | **画完整驾车路线**（起点→途经点→终点），路线本身是体验 |
| `city` | 活动集中在同一个城市；步行/地铁/公交 | **点位地图**：标注景点、餐厅、酒店，不画路线 |
| `transfer` | 主要是火车/飞机从 A 城到 B 城；到达后活动很少 | **不画地图**，用文字卡片展示交通信息 |
| `mixed` | 上半天移动（火车/飞机），下半天城市游玩 | **拆开**：交通段用文字，城市段用点位地图 |

### 如何判断

- 出现「租车」「自驾」「开车」「drive」「Passo」「山口」 → `drive`
- 出现「步行」「地铁」「散步」「city walk」，或活动全在一个城市 → `city`
- 主要内容是「A → B」的交通，到达后只有「放行李」「吃晚饭」 → `transfer`
- 上半天移动 + 下半天有 3 个以上活动 → `mixed`

### 同一天可能混合

例如「上午从威尼斯坐火车到多洛米蒂，下午自驾看山」→ `mixed`，交通段（火车）用文字，自驾段画路线。
