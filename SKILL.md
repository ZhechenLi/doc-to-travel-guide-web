...
2. **Notion 页面链接**：用户给出 URL，Agent 通过 MCP / 浏览器读取页面内容，再归类。Notion 数据库的建议列名见 **[references/input-constraints.md](references/input-constraints.md)**。
...
0. **读取输入** — Markdown 文件直接读取；Notion 链接通过 MCP / 浏览器获取内容。
