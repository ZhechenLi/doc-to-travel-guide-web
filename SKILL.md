...
0. **读取输入** — Markdown 文件直接读取；URL（Notion / Google Sheets / 任意网页）走**逐级降级管道**：浏览器获取 HTML → 尝试转 MD → 信息不够则截图或 PDF → 最后才用平台 API。详见 [table-source-handling.md](references/table-source-handling.md)。
...
3. **生成单页 HTML** — 见 [references/html-patterns.md](references/html-patterns.md)。**视觉原则：简洁、去 AI 味**——不要渐变彩虹、不要 emoji 堆砌、不要「祝你旅途愉快」之类的 AI 套话。排版像一个人自己做的旅行手册，不像机器生成的模板。
4. **地图规则** — 详见 [references/html-patterns.md](references/html-patterns.md) 地图章节。核心：**按城市拆图，跨城不混画**。同一城市内的活动共享一张地图；城市与城市之间的交通（火车/飞机）不画在地图上，用文字卡片表达；**唯一例外**：自驾行程中城际路段本身是体验的一部分（如多洛米蒂山路），此时画完整驾车路线。Google 链接处理见 [references/google-maps-embed.md](references/google-maps-embed.md)。
